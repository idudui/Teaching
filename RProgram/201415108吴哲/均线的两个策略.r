  ...
  for(i in 1:r){
    if(i<22){#赋初值
      ...
      if(IBM[i-2,l]>=IBM[i-2,k]&&IBM[i-1,l]<IBM[i-1,k]){##买点
        leibiao[i,1]=1##买信号
        leibiao[i,3]=leibiao[i-1,4]%/%IBM[i,l]##买股票数
        leibiao[i,4]=leibiao[i-1,4]-IBM[i,l]*leibiao[i,3]##手中现金
      }
      if(IBM[i-2,l]<=IBM[i-2,k]&&IBM[i-1,l]>IBM[i-1,k]){##卖点
        if(leibiao[i-1,3]>0){
          leibiao[i,2]=-1##卖信号
          leibiao[i,3]=0##卖完股票
          leibiao[i,4]=leibiao[i-1,4]+IBM[i,l]*leibiao[i-1,3]##手中现金
          }
        }
        }
    }
    if(i==r){##最后时刻平仓
      leibiao[i,1]=0
      leibiao[i,2]=0
      leibiao[i,4]=leibiao[i-1,4]+leibiao[i-1,3]*IBM[1,l]##平仓现金
      leibiao[i,3]=0
    }
    leibiao
  }
...
IBM=cbind(IBM,cl1,cl2)	##合并两个策略的数据
##加载程序包
library(TTR)
library(quantmod)
library(zoo)
